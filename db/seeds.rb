...
# real seed data, all airports in Colorado
...
for i in 1..50 do
  ...
  if @user.isExaminer == true
    # add examiner-only fields
    @user.update(bio: Faker::Lorem.paragraph(3, true), rates: rand(100..900))
    # seed airports for user (examiners only)
    3.times do
      AirportUser.create(
        user_id: @user.id,
